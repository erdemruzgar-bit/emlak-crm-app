...
export default function OccupancyTypesSettingsPage() {
  const confirm = useConfirm();
  const [items, setItems] = useState<OccupancyStatusCatalog[]>([]);
  ...
  async function deleteItem(id: string, label: string) {
    const ok = await confirm({
      title: `"${label}" durumunu sil?`,
      tone: "danger",
      confirmText: "Sil",
    });
    if (!ok) return;
    const res = await fetch(`/api/occupancy-types/${id}`, { method: "DELETE" });
    const data = await res.json();
    if (!res.ok) {
      toast.error(data.error || "Silinemedi");
      return;
    }
    toast.success("Sakin durumu silindi");
    load();
  }
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="bg-surface-container-lowest rounded-3xl p-12 text-center">
          <Home className="w-12 h-12 mx-auto text-on-surface-variant/40" />
          <p className="text-sm font-bold text-on-surface mt-4">Kayıtlı durum yok</p>
        </div>
      ) : (
