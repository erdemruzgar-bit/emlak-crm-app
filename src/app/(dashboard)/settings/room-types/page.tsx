    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      >
        <div className="flex-1 min-w-[180px]">
          ...
          <input
            placeholder="örn: 7+2, Dubleks, Tripleks"
            value={newName}
      ...
      {loading ? (
        ...
        <div className="bg-surface-container-lowest rounded-3xl p-12 text-center">
          <DoorOpen className="w-12 h-12 mx-auto text-on-surface-variant/40" />
          <p className="text-sm font-bold text-on-surface mt-4">Kayıtlı oda tipi yok</p>
          <p className="text-xs text-on-surface-variant mt-1">
            Yukarıdaki formdan ekleyebilirsin.
          </p>
        </div>
      ) : (
          ...
          {items.map((it) => (
            ...
            >
              <div>
                ...
                <p className="text-[10px] text-on-surface-variant">Sıra: {it.sortOrder}</p>
              </div>
              <button
